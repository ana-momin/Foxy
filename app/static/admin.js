  ...
  function refresh() {
    ...
    fetch(here.pathname + here.search, { headers: { "X-Partial": "1" } })
      ...
      .then(function (html) {
        ...
        ["figs", "wsp", "cap", "state"].forEach(function (id) {
  ...
  function send(form) {
    ...
    fetch(form.action, { method: "POST", body: data })
      ...
      .then(function (out) {
        ...
        form.reset();
        // reset() fires no event, so anything drawing from the form - the
        // announcement preview - would keep showing text that is now gone.
